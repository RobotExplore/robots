...
def forward_kinemacitcs(joints):
...
def ik(T_tar, joints_ini):
    ...
    iteration = 0
    while True:
        T_cu = forward_kinemacitcs(joints_cu)
        delta_p = T_tar - T_cu
...
end_ini = forward_kinemacitcs(joints_ini)
...
# end_positions[0:2]
# joints_rst.shape
end_positions_pred = np.array([forward_kinemacitcs(joints)
